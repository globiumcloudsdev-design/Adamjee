...
/**
 * Comprehensive test script for Student APIs (FIXED VERSION)
 * Tests:
 * - Student Dashboard API
 * - Student Fees Vouchers API
 * - Student Attendance API
 *
 * Usage: node test-student-apis-fixed.js
 */
...
async function testDashboardAPI() {
  ...
  // Test with invalid student ID - should handle gracefully
  console.log("\n--- GET /api/student/dashboard?studentId=invalid (permission check) ---");
  const unauthorizedRes = await requestJson(
    `${BASE_URL}/api/student/dashboard?studentId=not-a-real-uuid`,
    {
      method: "GET",
      headers,
    }
  ...
  logTest(
    "Dashboard handles invalid studentId gracefully",
    unauthorizedRes.status >= 400,
...
async function testFeesVouchersAPI() {
  ...
  // Test permission check with invalid student ID
  console.log("\n--- GET /api/student/fees-vouchers?studentId=invalid (permission check) ---");
  const unauthorizedVRes = await requestJson(
    `${BASE_URL}/api/student/fees-vouchers?studentId=not-a-real-uuid`,
    {
      method: "GET",
      headers,
    }
  ...
  logTest(
    "Fees vouchers handles invalid studentId gracefully",
    unauthorizedVRes.status >= 400,
...
async function testAttendanceAPI() {
  ...
  if (attendanceRes.ok && attendanceRes.body.data) {
    const { attendances, records, summary, statistics, pagination } = attendanceRes.body.data;

    logTest(
      "Attendance records is array",
      Array.isArray(records) || Array.isArray(attendances),
      `Count: ${records?.length || attendances?.length || 0}`
    );

    logTest(
      "Attendance statistics present",
      !!(statistics && typeof statistics.totalRecords !== "undefined"),
      `Total: ${statistics?.totalRecords}, Present: ${statistics?.presentDays}, Absent: ${statistics?.absentDays}`
    );

    logTest(
      "Attendance percentage calculated",
      !!(statistics && typeof statistics.attendancePercentage !== "undefined"),
      `Percentage: ${statistics?.attendancePercentage}%`
    );

    if (records && records.length > 0) {
      const firstAtt = records[0];
      logTest(
  ...
  logTest(
    ...
    `Records in range: ${rangeRes.body?.data?.records?.length || 0}`
  );
  ...
  logTest(
    ...
    `Present records: ${statusRes.body?.data?.records?.length || 0}`
  );
